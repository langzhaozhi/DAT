 ...
 * 根据此定义，多模式串匹配将匹配出所有<前缀前匹配prefixBeforeMatch>和所有的<后缀前匹配suffixBeforeMatch>(参见<code>DoubleArrayTriePrefixMatcher</code>中的概念定义)，
 * 还包括完整精确匹配的结果，以及其他所有关键字串是输入字串的子串的数据结果。之所以前缀后缀匹配的结果有限而非所有完整的前缀后缀结果，从定义上认清谁的子串就清楚了。
 * 如果想要<前缀后匹配prefixAfterMatch>或<后缀后匹配suffixAfterMatch>，那么应该使用<code>DoubleArrayTriePrefixMatcher</code>或其对偶。</p>
